...
- Files: `core/checks/power_accuracy.py::modulator_setup()` (full sequence: `top` / `modulator-config` / `line` / `tx enable` / `sine off` / `symbol-rate` / `roll-off` / `dual-channel-mode single-ch` / `-channel-1` / `state enable` / `source test-pattern` / `modulation qpsk` / `frame-size normal` / `fec-rate 2/3` / `pilot yes`), possibly a new shared helper in `core/checks/base.py` if this sequence is likely to be reused, but scoped to `power_accuracy.py` only for now since neither `flatness.py` nor `iq_validation.py` use this signal path.
- Config keys (additive): `power_accuracy.roll_off`, `modulation`, `fec_rate`, `frame_size`, `pilot`, all with defaults matching legacy's literal values (`0.25`, `qpsk`, `2/3`, `normal`, `yes`) so an old config still runs with legacy-equivalent behavior once these keys are code-defaulted. Plus, per decision 4: a new `power_accuracy.set_output_level_mode` key (default **`true`** — not opt-in) that makes `modulator_setup()` explicitly set Output Level Mode to Constant-Power, rather than relying on inherited DUT state as legacy does. Rationale: we have documented evidence the DUT carries state across sessions (that's exactly how it was found in NS4 / Roll Off 0.25 / Single channel / Constant-Power without the app setting any of it) — legacy could rely on inheritance because it was a single-purpose tool, but our app runs three checks that each reconfigure the unit, so determinism outweighs literal parity here. The key stays present so it *can* be disabled, but defaults ON.
...
- Risk: **Medium** — direct behavior change to DUT signal generation (modulation format, FEC, pilot), should be bench-verified independently of Stages 1–3 so any effect on the numbers is attributable to this stage specifically, not conflated with the CXA-side fixes.
- Bench check: with Stages 1–3 in place, confirm the DUT reports the expected modulation/FEC/pilot state (e.g. via its own CLI query) matches what legacy configured, and check whether any residual deviation changes.
